    ...
    public Flux<DetailedDrop> followDetailedDropperWithError(int greenHouseId, int rowId, int dropperId) {
        ...
        return GreenHouseProducer.getDrops()
                .filter(drop -> drop.getGreenHouseId() == greenHouseId &&
                                drop.getRowId() == rowId && drop.getDropperId() == dropperId)
                .flatMap(drop -> getDetailedDrop(drop)).onErrorResume(Exception.class,
                        e->Mono.error(new ResponseStatusException(404, "No such information found", e)));
    }
}
